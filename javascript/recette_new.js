...
function supprimer(id){
	...
	xhr.open("POST", document.location.href + '&ajax_action=supprimer', true);
	xhr.setRequestHeader('Content-type', 'application/x-www-form-urlencoded');

	xhr.send("id="+id);
}

function edit(id, mesure, nom){
	var quantite;
	switch(mesure){
		case 'U':
			quantite = prompt('Combien y a t\'il de ' + nom + ' ? ');
			break;
		case 'P':
			quantite = prompt('Combien de kilos de '+ nom + ' ?');
			break;
		case 'L':
			quantite = prompt('Combien de litres de ' + nom + ' ?');
			break;
	}
	...
	xhr.open("POST", document.location.href + '&ajax_action=modifier', true);
	xhr.setRequestHeader('Content-type', 'application/x-www-form-urlencoded');
